...
- Phase 5.6 (in progress): Historical Data Warehouse — planning card `t_90ca9e6f` Done. 2 implementation cards Done (`t_1c8a70da` — MarketDataProvider interface, commit `808ec02`; `t_b2a75ee8` — Local historical warehouse layout, commit `c2173c9`). 1 implementation card in Review (`t_56f319a9` — Data catalog extension, commit pending). 9 remaining implementation cards Ready. Downstream unblocks after #3: `t_e9626fc3` (duckdb_queries), `t_c32b8416` (data_versioning), `t_ababb2f7` (validation), `t_b06ec41d` (gap_detection), `t_31308fc2` (import_pipeline), `t_cf80bf36` (incremental_sync), `t_e6bf82a9` (research_cache). Non-blocking for v1.0. Storage architecture: DuckDB + Parquet + SQLite (hybrid). Design doc: `docs/architecture/phase-5-6-historical-warehouse.md`.
