...
- Phase 5.6 (in progress): Historical Data Warehouse — planning card `t_90ca9e6f` Done. 1 implementation card Done (`t_1c8a70da` — MarketDataProvider interface, commit `808ec02`); 1 implementation card in Review (`t_b2a75ee8` — Local historical warehouse layout, commit pending). Downstream unblocks after #2: `t_56f319a9` (catalog), `t_6168af8e` (parquet_storage), `t_22210825` (provider_plugins already unblocked by #1). Non-blocking for v1.0. Storage architecture: DuckDB + Parquet + SQLite (hybrid). Design doc: `docs/architecture/phase-5-6-historical-warehouse.md`.
