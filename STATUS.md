...
- Phase 5.6 (complete): Historical Data Warehouse — planning card `t_90ca9e6f` Done. **All 12 implementation cards Done.** Read + write + validation + cache path fully wired.  Warehouse-first `WarehouseReader` with provider priority + version pinning; ``ResearchAccountClient.fetch_bars`` provably not called when local coverage is complete.  Non-blocking for v1.0. Storage architecture: DuckDB + Parquet + SQLite (hybrid). Design doc: `docs/architecture/phase-5-6-historical-warehouse.md`.
