...
- Phase 5.6 (in progress): Historical Data Warehouse — planning card `t_90ca9e6f` Done. 2 implementation cards Done (`t_1c8a70da` — MarketDataProvider interface, commit `808ec02`; `t_b2a75ee8` — Local historical warehouse layout, commit `c2173c9`). 10 remaining implementation cards Ready. Cards now unblocked: `t_56f319a9` (catalog), `t_6168af8e` (parquet_storage), `t_22210825` (provider_plugins). Non-blocking for v1.0. Storage architecture: DuckDB + Parquet + SQLite (hybrid). Design doc: `docs/architecture/phase-5-6-historical-warehouse.md`.
