...
- Phase 5.6 (in progress): Historical Data Warehouse — planning card `t_90ca9e6f` Done. 3 implementation cards Done (`t_1c8a70da` — MarketDataProvider interface, commit `808ec02`; `t_b2a75ee8` — Local historical warehouse layout, commit `c2173c9`; `t_56f319a9` — Data catalog extension, commit `0fd96dd`). 1 implementation card in Review (`t_6168af8e` — Parquet bar storage, commit pending). 8 remaining implementation cards Ready. Downstream unblocks after parquet: `t_e9626fc3` (duckdb_queries), `t_ababb2f7` (validation). Non-blocking for v1.0. Storage architecture: DuckDB + Parquet + SQLite (hybrid). Design doc: `docs/architecture/phase-5-6-historical-warehouse.md`.
